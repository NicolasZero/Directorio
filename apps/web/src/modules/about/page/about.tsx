...
import { Heart, Globe, Calendar, Target, Lightbulb } from "lucide-react"
...
const About = () => {
  ...
  if (loadingInstitution) return <div className="min-h-screen flex items-center justify-center">Cargando...</div>
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="py-16 px-4 bg-muted/30">
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <Card className="hover:shadow-lg transition-shadow">
            ...
            <CardContent>
              <p className="text-muted-foreground text-lg">{institucion.mision}</p>
            </CardContent>
          </Card>
          ...
          <Card className="hover:shadow-lg transition-shadow">
            ...
            <CardContent>
              <p className="text-muted-foreground text-lg">{institucion.vision}</p>
            </CardContent>
          </Card>
        </div>
      </section>
      ...
      {/* Logros */}
      <section className="py-10 px-4 bg-rose-600 dark:bg-rose-900 text-white">
        <h2 className="text-center text-3xl font-bold mb-3">Nuestro Impacto</h2>
        <p className="text-center text-rose-100 dark:text-rose-300 max-w-xl mx-auto mb-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {institucion.logros.map((logro, index) => (
            <div key={index} className="text-center">
              <h2 className="text-4xl md:text-5xl font-bold mb-2">{logro.numero}</h2>
              <h3 className="text-rose-100 font-medium">{logro.titulo}</h3>
              <p className="text-rose-200 text-sm">{logro.descripcion}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="py-10 px-4 max-w-3xl mx-auto">
        <h2 className="text-center text-3xl font-bold mb-3">Nuestra Historia</h2>
        <Card>
      ...
      {/* Entes relacionados */}
      <section className="py-10 px-4 bg-muted/50 flex flex-wrap gap-6">
        {institucion.aliados.map((aliado, index) => (
          <Card key={index} className="hover:shadow-md transition-shadow max-w-[280px] mx-auto">
